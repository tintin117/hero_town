...
func _init() -> void:
	...
	assert(g.save_game("res://.godot/test_outputs/test_save.json"))
	var loaded=Rules.new()
	loaded.load_game("res://.godot/test_outputs/test_save.json")
	assert(loaded.s.battle.won and loaded.s.owned==2)
	loaded.s.last = Time.get_unix_time_from_system()-600
	assert(loaded.save_game("res://.godot/test_outputs/test_save.json"))
	var offline = Rules.new()
	var summary = offline.load_game("res://.godot/test_outputs/test_save.json")
	...
	DirAccess.remove_absolute("res://.godot/test_outputs/test_save.json")
