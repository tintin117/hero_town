...
> **Historical prototype reference.** The September 2026 remaster now runs through `scripts/remaster/`. See [REMASTER.md](../TOWN.md) for the current architecture, save format, controls, and tests. The runtime description below documents the retired prototype and must not be used to reconnect its combat or preview panels to the active game.
...
`game/town/town.tscn` is the one and only game scene.
...
[autoload]
GameData            = game/town/game_data.gd
GameState           = game/town/game_state.gd
fx                  = vfx/fx.gd
sfx                 = game/shared/sfx.gd
...
1. **`GameData`** (`game/town/game_data.gd`) scans `res://data/town/heroes/`,
   `res://data/town/enemies/`, `res://data/town/buildings/` for `.tres` resource files and
...
2. **`GameState`** (`game/town/game_state.gd`) holds the player's live save-state:
...
4. **`sfx`** (`game/shared/sfx.gd`) is a tiny sound-effect bus: an 8-voice pool of
...
Then `game/menu/main_menu.tscn` loads and shows the menu.

### Menu → gameplay

`game/menu/main_menu.gd` wires three buttons:

- **Play** → `get_tree().change_scene_to_file("res://game/town/town.tscn")`
...
`game/town/town.tscn` is the actual, current game scene — a `Node2D` world built
...
### `game/town/game_state.gd` — `GameState`
...
### `game/menu/animation_button.gd` — class `AnimationButton extends TextureButton`
...
|---|---|---|---|
| `data/town/schemas/hero_data.gd` | `HeroData` | id, display_name, rarity, hero_class, unit_type, base_power, power_per_level (unused — no per-hero leveling anymore), base_hp, atk_speed, mana_per_hit | `data/town/heroes/h001–h015.tres` |
| `data/town/schemas/enemy_data.gd` | `EnemyData` | id, display_name, tier, unit_type, hp, atk, spawn_time (unused by any code today), gold_min/max, shard_min/max | `data/town/enemies/e001–e006.tres` |
| `data/town/schemas/building_data.gd` | `BuildingData` | id, display_name, build_cost, unlock_th_level (unread by any code — a dead gate, same status as before), thumbnail, sprite_texture, model_scene (unused, pre-2D leftover), levels (Array[Dictionary] — per-level cost/tier/spawn data), hero_class + reward_pool (only meaningful for hero-producing buildings) | `data/town/buildings/{town_hall,portal,barracks,blacksmith,tavern}.tres` |
...
(`data/town/buildings/shrine.tres`, `scenes/shrine_popup.tscn`) no longer exists —
