...
Commands and scene responsibilities are in [REMASTER.md](../TOWN.md). When running inside a filesystem sandbox, supply a separate writable log path for each process, for example:

```powershell
& $godot --headless --path . --log-file '.godot/remaster-tests.log' res://tests/town/rules_checks.tscn -- --test
```
