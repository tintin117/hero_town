...
2. Three Warriors appear immediately. The preparation timer starts when the first army exists; **Start now** skips the wait.
3. Battles run automatically. Victory awards gold once, and all soldiers recover between rounds.
4. Click a building, or choose **Research** and select an army by its numbered identity. Each building has independent Crew, Rarity, and Training branches.
...
Right-click or Escape cancels placement. Moving is free and changes the next round's formation. Full research refunds are available during preparation; the building remains. Purchases during combat never alter the active battle snapshot.
...
Offline income is half the average ordinary-battle gold rate across the latest five successful ordinary battles. Each sample includes the full 20-second preparation and five-second recovery, even if Start now skipped the countdown. First-clear bonuses are excluded. Earnings cap at eight hours, require a prior ordinary victory, and never unlock stages. Research refunds clear the measured rate. Returning applies and saves gold before showing the summary; repeated reopening and clock rollback cannot repeat the reward.
...
Always pass `-- --test` when running test scenes. This disables loading the real player save; save tests use isolated files under `.godot/remaster_tests/`.
...
The balance script simulates an explicit purchasing policy; its times are pacing evidence, not a guarantee for every player strategy. It reserves gold for unlocked army types, then favors affordable crew, promotions, and training. A full baseline run finishes near 65 minutes with the first captain near nine minutes. Human playtesting should assess readability, sound levels, formation clarity, and whether the later captain battles feel rewarding.
