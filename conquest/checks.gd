...
func _init() -> void:
	...
	assert(g.save_game("user://conquest-test-save.json"))
	var loaded=Rules.new()
	loaded.load_game("user://conquest-test-save.json")
	assert(loaded.s.battle.won and loaded.s.owned==2)
	loaded.s.last = Time.get_unix_time_from_system()-600
	assert(loaded.save_game("user://conquest-test-save.json"))
	var offline = Rules.new()
	var summary = offline.load_game("user://conquest-test-save.json")
	...
	DirAccess.remove_absolute("user://conquest-test-save.json")
