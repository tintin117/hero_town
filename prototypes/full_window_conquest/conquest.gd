...
@export var balance: ConquestBalance = preload("res://data/companion/default_balance.tres")
var game = Rules.new()
var ui: CanvasLayer
var world: Node2D
var army: Node2D
...
func _ready() -> void:
	...
	if "--capture-conquest" in OS.get_cmdline_user_args():
		...
		get_viewport().get_texture().get_image().save_png("res://.godot/test_outputs/preview.png")
